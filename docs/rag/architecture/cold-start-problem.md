...
## The mirror: why everything does not become money

Free entry creates candidates, not acceptance networks. A stationary monetary equilibrium establishes that coordinated acceptance is possible; it does not select the realized candidate or describe the transition into it. Prescott and Rios-Rull's OLG paper is sometimes cited as proving that rival private issuance destroys monetary equilibrium, but it does not: its standard monetary allocations fail an added no-restarting refinement, and its replacement organizational equilibrium exists.

The transition has two mechanisms, not one. **Bootstrap comes first:** where an incumbent exists, its advantage is installed deep in counterparties, prices, contracts, accounting, payroll, tax, custody, regulation, habits and skills. It also carries inherited beliefs about legitimate monetary origins, trusted administrators, stable worth and proper issuance. Network utility plus these infrastructural, behavioural and epistemic switching costs form an adoption premium the challenger must outbid **for a specific function or use-domain**. A bootstrap engine pays, compels or otherwise supplies enough extra demand to establish a viable early cluster. A challenger can also borrow legitimacy by mimicking a familiar trust grammar—reserves, redemption, mining or gold-like scarcity. This **gold simulacrum** lowers epistemic switching costs; it is not a value source, and the older belief can coexist with the new practice. **Cascade comes second:** in Watts's threshold model most viable seeds still die; a global cascade requires the seed to reach a connected vulnerable cluster inside a limited cascade window. Network effects therefore change sides within the relevant domain. Initially they defend the incumbent; beyond critical mass they amplify the challenger; once it is installed, they and switching costs defend its position (Luther 2016). Thus cold start asks **what funds and legitimates a foothold for this job, and can that foothold propagate?** The outcome need not be one winner: selection occurs among units, ledgers, instruments and rails inside particular situations and relationships. Phone and card can be rival interfaces to the same ledger; cash supplies bearer settlement. A meal can clear by phone or card while its tip clears in cash. Casino chips, concert tokens and coupons can displace general money inside bounded acceptance networks. People can use all of them while holding incompatible monetary beliefs at once. Bootstrap engines pay the adoption premium; they do not inject worth into the ledger.

## Siphon priming and continuation

Cold start is a **siphon-priming cost**: somebody must remove the air and create the first continuous acceptance path. Bootstrap pays for the initial cluster; cascade extends it. Maintenance is a different mechanism. Once future trade, credit, cooperation and specialisation depend on the installed record, expected loss of those opportunities can motivate performance and distributed keepership by ordinary users—authentication, correct change, reporting, challenge, future refusal, forgiveness and repair—without one issuer or regulator doing every job.

This is conditional self-enforcement, not an automatic property of acceptance. For each actor, expected continuation surplus at risk, multiplied by the chance that defection is observed and consequentially answered, must exceed the one-shot defection gain plus participation and keeper cost. Failure means continuing subsidy, duty, office, coercive support or collapse; it does not disappear behind the label *self-sustaining*.
