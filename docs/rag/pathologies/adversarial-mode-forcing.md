...
Unlike [mode collision](../architecture/mode-collision.md) (a symmetric mismatch between peers), adversarial mode forcing is _asymmetric_: a system (algorithm, employer, platform) imposes a fairness mode on a user who had no negotiating power and often no awareness that the choice was being made.
...
The most insidious version: a transaction that _presents_ as one mode while _operating_ as another.
...
1. **Name the basis** — _"You are pricing me individually based on data about me."_ Once named, the imposition has to be justified.
2. **Refuse the frame** — _"I will not engage with this in Value mode. This is a different kind of relationship."_ Walk if the other side won't shift.
3. **Invoke a different mode** — _"This belongs in Balanced mode. We have a long-standing relationship."_ Or _"This should be Obligated — there should be a single rate for everyone."_
