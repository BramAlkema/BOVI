...
2. **There was no transition from commodity money to abstract/fiat money.** It was a ledger from the first circulating divisible-known-supply good. What changed over history wasn't substance becoming abstraction — it was the **substrate getting thinner** (cattle → metal → coin → note → book → chain), each a better place to keep the *same* ledger. The "abstraction" is just shedding a superfluous carrier. Two calibrations on the climb to the `τ = 1` limit where only the record remains: the endpoint is a *mixture*, not a destiny — Kocherlakota & Wallace (1998) show that with imperfect record-keeping the optimum uses money *and* memory together, the composition tracking record-keeping quality — and the gold reading is the framework's structural claim carried by Menger's saleability, not a corollary of the theorem, whose "money" is defined as entering no utility function (gold does; his one extension covers only "some small amount" of direct utility). *The ledger reading of commodity money stands on the identity above, at that strength and no more.*

**What Kocherlakota's theorem actually says, and where the identity really comes from.** The theorem (1998) is an *inclusion*, not an equivalence: any allocation money can support, memory can support — his section title is "Money Is No Better Than Memory," his phrase is that money is "a *primitive form* of memory," and the converse holds in overlapping-generations and turnpike environments while **failing in random matching among strangers**, where memory strictly dominates (his Example 2 — and stranger-matching is this framework's own setting). The identity table above is not a sharpening of him; it is his own proof intuition, stated on p. 233: *"an imaginary balance sheet is kept for each agent… money is merely a physical way of maintaining this balance sheet."* He reads it deflationarily (money as the primitive record); this document reads the same identity from the token side. The framework's own contribution is the τ-decomposition of marketability into ledger-vs-commodity properties — as the pedigree above already says — not the identity.

**The honest form: money is a forgetful ledger with a hard floor at zero.** Two things the token version gives up relative to a full record: balances cannot go negative (so the token ledger extends no credit — you cannot hold −5 coins), and entries cannot condition on identity or history (which is exactly why memory strictly dominates in Example 2). And one thing the floor *buys*, now experimentally attested: it is a hard budget constraint no negotiation can soften — Bigoni, Camera & Casari ("Money is more than memory," *JME* 2020) find money *outperforming* memory in the lab, because under memory free-riders ran unbounded deficits, and money's advantage collapses exactly when the liquidity constraint is removed. The floor is Stone 4's enforcement row in laboratory form: possession-is-settlement means nobody can owe, which is why the token form needs no teeth — and why lifting the floor (mutual credit, signed balances) re-opens the enforcement question. Counterweight, from the source's own Example 3: *unlimited* memory can be worse than money — the inclusion depends on memory being limited — so the token is not simply a degraded ledger on one monotone axis.

**Scope note:** reading holdings as "net claims" requires a numeraire, and the theorem supplies none — Kocherlakota: "there is no immediate technological reason why money should be a better numeraire than other goods." The identity is therefore a claim about the *Value channel*, where prices exist — consistent with the framework's own scoping of money to the clear-it-now discipline, and stated here so the table cannot be read wider than that.
