...
- **ADICO → Solidity.** Frantz & Nowostawski, *From Institutions to Code: **Towards** Automated Generation of Smart Contracts* (IEEE FAS\*W 2016, DOI 10.1109/FAS-W.2016.53), takes Crawford & Ostrom's grammar of institutions — Attributes, **Deontic**, aIm, Conditions, Or else (*A Grammar of Institutions*, APSR 89:582–600, 1995) — toward Solidity constructs.

  ✅ **Citation verified** (title, authors, venue, DOI; 218 citations as of Aug 2026). ⚠️ **The mapping itself is not.** The abstract and full text are paywalled and unretrieved, and an earlier version of this entry asserted "automated translation from ADICO 'programs' to contracts" as fact — that characterisation came at one remove, not from the paper. Two reasons to hold it loosely until someone reads it: the title hedges with *Towards*, and Ciatto et al. (below), who did read it, credit it as "an interesting first step towards re-interpreting smart contracts and blockchains as virtual institutions" while noting its demonstration is a voting scenario that counts and triggers sequential activities rather than coordinating. Treat it as a design sketch with a proof of concept until shown otherwise. **Library pull.**

  **Why it matters here regardless, and why it would not have saved us.** ADICO supplies by name the category this canon was missing: the **D** is a deontic operator — *may / must / must not* — and this register's four classes had no row for a duty until `Ostrom` was built. So the vocabulary existed in 1995 and a route to code was attempted in 2016, in the prior-art section of the very document that could not see the keeper. But a complete mapping would still not have closed the gap, and the reason is instructive. ADICO's **O** — the *or else* — is the sanction, and a sanction requires a party to apply it; in Crawford & Ostrom the "or else" is itself an institutional statement, so the structure nests rather than bottoms out (⚠️ recalled, not retrieved). That is precisely where `Ostrom.sol` lands: it can record a duty and event its breach, it cannot execute a *must*, and its own flag needs a caller. A notation can express an obligation. A platform whose only primitive is *may call* cannot run one. The gap is real at the notation layer and the repair belongs to the specification-language work, not to another contract.
