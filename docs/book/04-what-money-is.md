...
4b. The banks as the living proof: nearly all modern money is commercial-bank ledger entries, created in lending and destroyed in repayment (loans create deposits — Bank of England 2014); banking is ledgers clearing against ledgers across the central bank's master ledger (Mehrling's money view). The ledger thesis on its home turf — and elastic supply ("known, not fixed") at industrial scale. **Kocherlakota's boundary line installed here**: money provides memory; a bond provides memory *and commitment* — and the bank deposit stands on both sides at once (deposit = memory, loan behind it = commitment; the reflux, 1844, named without footnotes). Feeds Ch 6's born-pre-sold and Ch 13's clearing-discipline read.
...
- **The clearing-discipline identity is stated here** (seeded in Ch 2 via Fiske's counting observation): a mode is a rule about when and whether the ledger must clear *and who it falls to*; money is the Value discipline given infrastructure. The bearer clause was absent from every statement of this identity until 2026-08, while the fieldbook card had always asked *and says who?* — an agentless *must* is the grammar the book exists to refuse, so it cannot stand in the book's own hinge. Returns in Chs 6, 8A, 10, 12, 16, 19, 22, 22A and Epilogue A.9 — the joint between the modes half and the money half of the book.
...
Kocherlakota's proof came with a boundary line, drawn in a sentence that deserves to be more famous than his slogan: money only provides memory, but a *bond* provides memory **and commitment.** A record of who has given, on one side; the same record *plus an enforceable promise that someone, on a date, must square it*, on the other. Keep the line in a pocket — the rope sits on one side of it, a tax bill and a loan contract on the other — because in a moment it will turn out that most modern money contrives to stand on both sides at once.
...
And notice what such an entry is, in the vocabulary of a page ago: both sides of Kocherlakota's line at once. The deposit is memory — a balance that circulates, remembering who has given. The loan behind it is commitment — a dated, enforceable obligation that will one day call the deposit home and un-write it. Nearly all modern money is a memory fused, at birth, to the promise of its own erasure. The oldest name for the pattern comes from a banker's fight in 1844 — notes issued on loan must return when the loan falls due, *the reflux and the issue balancing each other* — and it will matter again when we ask which clearing discipline this is, because a schedule with a penalty has a mode, and it is not the market's.
...
What distinguishes the modes, then, is not whether the books are open but *what discipline of clearing* each one keeps. Look again at the four questions from Chapter 2 and notice they are all questions about when, and whether, the books must square — and, underneath that, about who says so. Value clears *now*, and the counter says when: pay the price and walk away square. Balanced *nets over time*: nobody is in debt, exactly; the books just lean, and the two of you are trusted to right them. Obligated clears *on a schedule someone above you set*, with a penalty for missing it. And Immediate *never clears at all* — in the mode of love, presenting the account for settlement is the insult, and the person who presents it has said something about the relationship they cannot take back.

A mode, it turns out, is a rule about when the ledger must clear — and every such rule falls to somebody. Keep that second half. It is easy to lose, because a rule reads stronger without it: *the books must square* sounds like a law, while *the books must square, and it falls to you to square them* sounds like a chore. But the version that sounds like a law is the weaker one, and it is weaker in a specific way. Strike out who a duty falls to and you have not discovered a law of nature. You have only lost track of who is holding the pen — and someone is always holding the pen. That is the whole method of this book applied to its own definition: whenever a sentence about money seems to work without anybody in it, look again, because the person has not gone anywhere. They have just stopped being visible, which is generally who benefits. Money — the coin, the note, the balance on the card — is what happened when one of those four disciplines, the clear-it-now kind that works between strangers, got its own infrastructure. The other three kept their books where they had always kept them: in memory, in rank, in love. Hold this thought; the whole book is in it, and near the end we will build the tool that keeps all four sets of books at once.
