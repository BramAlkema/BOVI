...
Two compressions the recursion earns once the future-obligation literature is read against it. First, make the survival hazard explicit: let `ℓ` be the per-period liquidity service and `δ` the per-period probability that the acceptance network dies (the worthless equilibrium, always available, being reached). In steady state the sum collapses to a perpetuity:

```
V = ℓ / (r + δ)
```

— the value of money is the flow of unlocked marginal surplus, over the discount rate plus the network-death hazard. Each holder harvests one period's `ℓ` and sells the remaining stream to the next. Every comparative static the body uses falls out: network thickness raises `ℓ` for every holder at once (the pull, self-reinforcing); a larger stock meets ever-more-marginal trades, so `ℓ` falls at the margin (quantity discipline, derived rather than assumed); a credibility shock raises `δ` and cuts `V` with `ℓ` untouched. The margin is observable: holders add real balances until marginal `ℓ` equals the foregone nominal rate, so the market quotes the service's price daily.

Second, the fraction locates the entire obligation family — tax receivability, loan repayment, fiscal backing — in exactly one slot, and it is not the numerator. A tax discharges at face value whatever the price level: it adds no service flow. What a legally pre-committed acceptor does is **truncate `δ`**: it makes the finite-horizon unravelling (Starr's theorem, with its capacity condition — the take must cover the stock) unreachable, the way a guarantee changes a bond's survival probability without touching its coupon. `ℓ` is the value-source; `δ` is where selection and termination live. The fiscal theory of the price level then becomes legible as the same present-value machine pointed at a different stream: it discounts the *state's* primary surpluses where this equation discounts the *traders'* trade surpluses. The century's dispute over "backing" was always a dispute over whose surplus stream sits in the numerator — and the registers resolve it: the traders' stream is the worth; the state's stream is a guarantee on the denominator.
...
**And nothing in it can act without an event source.** Every clearing in the machinery waits for some person or service to observe and call. The code can constrain *whether* a clearing is valid; it cannot compel *that one happens*. The first audit therefore looked for one unnamed keeper. The deeper result is a topology: counterparties can authenticate, ordinary users can trigger, affected peers can report, a quorum can find, future counterparties can respond, and an office can maintain a scheduled function. Those jobs may be divided in the same institution. The honest demand is not one name at the end of the chain but a bearer, motivation, clock, fallback and failure path for every edge.

That changes the cold-start question too. The first connected path still has to be primed—someone bears the cost of drawing the air out of the siphon and giving an early cluster a reason to outbid the incumbent in one use. Once the path is installed, lost future trade, credit, cooperation and specialisation may motivate users to keep their portions of it going. *May* is load-bearing. The continuation surplus has to reach the person being asked to pull, exceed defection and upkeep, and survive uncertainty and exit. No contract manufactures that future opportunity, and no score makes people care about it.
...
They could be held the same way, and this is the one proposal in this book that runs ahead of what is built. Not by computing them—nothing computes whether preferences are stable. By **contracting** them: each assumption stated with a scope, stake and named observation that would refute it; each result declaring which assumptions it rests on; and an accepted refutation making downstream dependence mechanically visible. Even that propagation needs an observer, claim, contest, finding and trigger. The existing bond is one contest topology, not enforcement. What is missing is the typed evidence lifecycle and graph.

Follow it far enough and it stops being a register and becomes a notation—one specification of an economic claim that can be proved, simulated with heterogeneous agents and deployed to people, with disagreements between those three treated as findings rather than errors. Modes would be **types** in it; acceptance would have domains and a lifecycle; obligations and powers would have parties and states; every executive edge would declare its keeper topology; findings would remain separate from responses; priming and continuation would be bonded assumptions. That is a programme, not a result, and it is written up separately. Two boundary families remain: who may enter a claim at all, and how world events are observed, triggered and made consequential. They can be distributed, priced or assigned to an office. They cannot be wished into the machine.
