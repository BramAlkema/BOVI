...
**V1 is now treated as the reference baseline, not the V2 architecture; Phase 0 still has to freeze and tag it.** It contains sixteen Solidity contracts and nineteen passing tests across three suites, but a green demonstrator is not an institutional closure or security result. The ground-up redesign, including the disposition of every contract, the distributed-keepership correction, the cold-start/continuation split, and the proof–simulation–deployment gates, is in [`V2-OVERHAUL-PLAN.md`](V2-OVERHAUL-PLAN.md). Thinker names remain lineage in V2; they no longer determine module boundaries.
...
| `Greif` | Avner Greif | attributed reputation memory that may inform later counterparties. The current global scalar is advisory and is replaced in V2 by contextual, contestable, expiring and repairable findings; memory is not itself enforcement | **written — replace in V2** |
| `Ellickson` | Robert Ellickson | *Order Without Law* — bounded custom, bilateral performance, breach claim, challenge, peer finding, repair and exit. It models distributed informal order without pretending the custom is fair or wiring a finding to automatic punishment | **new V2 blueprint** |
...
| `ChallengeBond` | *(crypto-native)* | a bonded, contestable-assertion topology. It can allocate an escrowed bond after a declared resolution; it does not make an assertion true, equalise access to challenge, or enforce a social consequence | **written — narrow in V2** |
...
- **Stone 4 — the axioms** (resolution + integrity; the skims live in the sufficiency-gaps): `Stigler` (known-terms gap — the one genuinely information-driven skim), contextual `Greif` memory plus counterparty-owned future responses (the credit-form performance residual), `Hayek` (the unit/known-ness), `BigoniCameraCasari` (the experimental test of the enforcement row: the token's floor at zero *is* possession-is-settlement, and lifting it is what makes continued performance a separate institutional question). *`Cantillon` was listed here as the known-supply gap and has been moved off this axis (audit, 2026-08): his losers are fixed-lease and fixed-wage holders, not the ill-informed, and QE was pre-announced and redistributed anyway. It now meters exposure on the **unit-of-account / nominal-contract** axis, where `Fisher` is the defence and it is the meter.*
...
| `Forstater` | Forstater (2005); Bundy (1979) | a tax obligation denominated in a currency obtainable only by selling labour ⇒ forced proletarianisation, the migrant-labour system, rural social structure | `Kocherlakota` (Stones 1–2 at regional scale: demand comes from an obligation to clear, not from substance), `Fiske` (an Obligated-mode demand imposed across a boundary), state assessment, collection and legal consequence rather than a `Greif` score | blueprint |
...
Current forced-build reading: **26 authority positions across 14 contracts, none of them a declared type.** Without `--force`, an unchanged artifact may lack an AST and the script silently skips it; that previously produced 24 across 13 and omitted `Ostrom`. A hand audit of the same cast found eight. That gap is the argument: `address` is one type for the payer, the steward, the reporter, the oracle and the person they act upon, so a position can exist everywhere and be counted nowhere. A missing field is a compile error; a missing category is silence.
...
Nineteen tests across three files — `test/CoreE2E.t.sol` (the bench engine), `test/Clark.t.sol` (the liquidation threshold in isolation), and `test/Ostrom.t.sol` (the office topology). All sixteen contracts compile under solc 0.8.20. Seven contracts currently have no executed test path: `BigoniCameraCasari`, `Cantillon`, `ChallengeBond`, `Fiske`, `KiyotakiWright`, `Starr`, and `Stigler`; passing V1 is therefore a reproducible baseline, not adequate V2 coverage.
