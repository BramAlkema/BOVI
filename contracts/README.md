...
| `Ostrom` | Elinor Ostrom | *Governing the Commons* (1990) — **the office**: who holds the crank, appointed how, answerable to whom, on what clock. Nothing in this cast can act on its own, so every clearing discipline waits on a party who had no contract, no vote and no name — and whose *timing* turned out to be distributional. Turns a missed clearing from an absence into an evented fact. Records; does not punish (the sanction is referred, per the `Greif` ruling). The *incentive* half was already built as `Kocherlakota.pokeRewardBps` — Alchian & Demsetz's monitor-takes-the-residual; what was missing was accountability | **built** |
...
| [`SYSTEM-DYNAMICS.md`](SYSTEM-DYNAMICS.md) | — | **the cast as a system**: stocks, flows, and where each loop closes. Two closed loops on-chain, both balancing, both in `Kocherlakota`; every other loop routes through an oracle, a reporter, or a person. Two rulings came out of it — reputation is **advisory** (wiring `Greif` into credit limits would drive a normative output from an adjudicative input with neither guard), and `Friedman.propose` now **requires a stated reason**, because governance was the only place in the cast that could act and the only one that recorded no discipline | **reference** |
