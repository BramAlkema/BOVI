...
const rentScenario = new Scenario("rent", "Rent Collection Analysis", "balanced", "Monthly rent payments create a ongoing relationship between tenant and landlord that involves multiple fairness modes.")
    ...
    .addInsights("Primary mode is Balanced because ongoing monthly exchanges require symmetric record-keeping", "Rent control policies often conflict between Value mode (market pricing) and Immediate mode (affordability intuitions)", "Landlord-tenant disputes usually center on which fairness mode should dominate the relationship");
const taxScenario = new Scenario("tax", "Tax Payment Analysis", "obligated", "Government taxation demonstrates how authority creates money demand through hierarchical obligation.")
    ...
    .addInsights("Tax resistance often occurs when Obligated mode lacks legitimacy from other modes", "Progressive taxation conflicts between Value mode (proportional) and Immediate mode (equal treatment)", "Tax compliance depends on Authority Ranking acceptance combined with Balanced mode service expectations");
const marketScenario = new Scenario("market", "Farmers Market Analysis", "value", "Direct commerce at farmers markets demonstrates pure market pricing with supporting fairness modes.")
    ...
    .addInsights("Local markets often emphasize Immediate and Balanced modes over pure Value mode efficiency", "Vendor relationships matter - people pay premiums for trusted sources despite Value mode alternatives", "Farmers markets resist corporate Value mode optimization in favor of community-oriented fairness");
const favorScenario = new Scenario("favor", "Neighborhood Favors Analysis", "immediate", "Direct reciprocity between neighbors demonstrates pure Immediate mode fairness with minimal institutional mediation.")
    ...
    .addInsights("Immediate mode works best in small, stable communities where reputation matters", "Monetizing neighbor relationships often feels wrong because it violates Immediate mode expectations", "Community resilience depends on maintaining Immediate mode networks alongside formal economy");
const groceryScenario = new Scenario("grocery", "Grocery Shopping Analysis", "value", "Supermarket transactions demonstrate how Value mode efficiency can conflict with other fairness intuitions.")
    ...
    .addInsights("Shrinkflation violates Balanced mode expectations - same price, less product feels like cheating", "Local grocery stores often succeed by emphasizing Immediate and Balanced modes over pure Value efficiency", "Supply chain disruptions reveal how Value mode optimization sacrifices other fairness modes");
