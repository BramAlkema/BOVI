 ...
 */
export async function checkRuleCompliance(): Promise<{
  compliance: number;
...
export async function simulateBrackets(income: number, cpi: number): Promise<BracketResult> {
  ...
  for (let i = indexedBrackets.length - 1; i >= 0; i--) {
    if (income > indexedBrackets[i].threshold) {
      ...
      const nextThreshold =
        i < indexedBrackets.length - 1
          ? indexedBrackets[i + 1].threshold - indexedBrackets[i].threshold
          : taxableAtThisRate;
 ...
 */
export async function switchButler(
  butlerId: string
): Promise<{ switched: boolean; activationTime: number }> {
