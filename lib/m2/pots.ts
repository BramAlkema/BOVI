...
export interface Pot { 
  ...
  priority: number; 
  balance: number; 
  rules: { min?: number; max?: number; lock?: boolean };
}

export interface SweepRule { 
  to: string; 
  amount: { type: "fixed" | "percent"; value: number }; 
  jit?: boolean; 
  due?: { cron: string };
}

export function applySweeps(income: number, pots: Pot[], rules: SweepRule[]) {
  let remaining = income;
  const ordered = [...rules].sort((a,b)=> 
    (b.amount.type==="fixed"?b.amount.value:Infinity) - (a.amount.type==="fixed"?a.amount.value:Infinity) 
  );
  
  for (const r of ordered) {
    const p = pots.find(x=>x.id===r.to); 
    if (!p || p.rules.lock) continue;
