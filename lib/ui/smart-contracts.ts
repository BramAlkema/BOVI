...
import { createSmartContract, type ContractClause, type SmartContract } from '../services/smart-contracts.js';
...
function showContractCreationModal(templateId: string, parties: string[]): void {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const contractParties = parties.map((_, i) => formData.get(`party-${i}`) as string).filter(Boolean);
