...
export const MissionDeckPlugin: UIComponentPlugin = {
  ...
  async mount(ctx: UIContext): Promise<UIInstance> {
    ...
    // Load mission data and build cards
    (async () => {
      try {
        ...
        stack.querySelectorAll<HTMLButtonElement>(".btn.primary").forEach((btn, i) => {
        ...
        // Wire "Change it" buttons to overlays
        stack.querySelectorAll<HTMLButtonElement>(".btn:not(.primary)").forEach((btn, i) => {
    ...
    function missionCard(title: string, status: string, whyText: string, actionRequired: boolean) {
      return `
        <div class="card">
