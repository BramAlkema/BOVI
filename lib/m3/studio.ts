...
export function renderStudio(flow: M3FlowSpec, mount: HTMLElement) {
  ...
  const width = layers.length * xGap + pad * 2,
    height = Math.max(...layers.map(col => col.length)) * yGap + pad * 2;
  svg.setAttribute("width", String(width));
  svg.setAttribute("height", String(height));
  const pos = new Map<string, { x: number; y: number }>();

  layers.forEach((col, i) =>
    col.forEach((n, j) => pos.set(n.id, { x: pad + i * xGap, y: pad + j * yGap }))
  );
  ...
  for (const n of flow.nodes) {
    ...
    stripe.setAttribute("fill", modeColour(n.type.split(".")[0] as Mode));
...
function toposort(flow: M3FlowSpec): NodeSpec[][] {
  ...
  let layer = S;
  const outs = new Map<string, string[]>(flow.nodes.map(n => [n.id, []] as const));
  for (const e of flow.edges) outs.get(e.from)!.push(e.to);
  ...
  while (layer.length) {
    ...
    for (const n of layer) {
      ...
      for (const m of outs.get(n.id)!) {
        ...
        if ((indeg.get(m) ?? 0) === 0) next.push(flow.nodes.find(x => x.id === m)!);
      }
    }
    layer = next;
  }

  // add any orphan cycles as last column
  const orphans = flow.nodes.filter(n => !seen.has(n.id));
  if (orphans.length) layers.push(orphans);
  return layers;
}

function modeColour(m: string) {
