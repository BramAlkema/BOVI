...
describe("Compute Benchmarking", () => {
  ...
  describe("single operation benchmarking", () => {
    ...
    it("benchmarks asynchronous operations", async () => {
      const minimumMeasuredDelay = 10;
      const scheduledDelay = 20;
      ...
      expect(result.averagePerIteration).toBeGreaterThanOrEqual(minimumMeasuredDelay);
    });
  ...
  describe("benchmark suites", () => {
    it("benchmarks multiple operations", async () => {
      const runFor = (durationMs: number) => {
        ...
        while (performance.now() - start < durationMs) {
          // Deliberately occupy the requested interval to make the ranking stable.
        }
        return durationMs;
      };

      const operations = {
        "fast-op": jest.fn(() => runFor(1)),
        "medium-op": jest.fn(() => runFor(4)),
        "slow-op": jest.fn(() => runFor(8)),
      };
    ...
    it("tracks suite performance metrics", async () => {
      const suite = await benchmarkEngine.benchmarkSuite(
        "metrics-suite",
        {
  ...
  describe("memory profiling", () => {
    it("tracks memory usage when available", async () => {
      ...
      const result = await benchmarkEngine.benchmarkOperation(
        "memory-test",
        () => {
