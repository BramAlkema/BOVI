...
describe('Rulers API', () => {
  ...
  test('switchRuler throws error for invalid ruler', async () => {
    await expect(switchRuler('non-existent'))
...
describe('Hamburger Sentinel', () => {
  ...
  test('createHamburgerBasket stores basket', async () => {
    const items = [
      { name: 'Big Mac', brand: 'McDonalds', size: '1pc', price: 4.50, usual: 4.20, location: 'London', date: '2024-01-15' },
      { name: 'Fries', brand: 'McDonalds', size: 'Medium', price: 2.20, usual: 2.00, location: 'London', date: '2024-01-15' }
    ];
  ...
  test('calculateHamburgerInflation computes price changes', async () => {
    const items = [
      { name: 'Burger', brand: 'Test', size: '1pc', price: 5.00, usual: 4.50, location: 'Test', date: '2024-01-15' }
    ];
    ...
    expect(inflation.current).toBe(5.00);
    expect(inflation.previous).toBe(4.50);
    expect(inflation.change).toBe(0.50);
...
describe('Storm Mode', () => {
  ...
  test('activateStormMode throws error for non-existent profile', async () => {
    await expect(activateStormMode('non-existent'))
      .rejects.toThrow(BoviAPIError);
  });
});