...
describe("Plural Indices Marketplace", () => {
  ...
  test("setDefaultIndex throws error for invalid provider", async () => {
    await expect(setDefaultIndex("non-existent")).rejects.toThrow(BoviAPIError);
  });
  ...
  test("calculateWithProvider returns calculation result", async () => {
    const result = await calculateWithProvider("bovi-local", [{ price: 2.5 }, { price: 1.2 }]);
...
describe("Butler Hub", () => {
  ...
  test("activateButler throws error for non-existent butler", async () => {
    await expect(activateButler("non-existent")).rejects.toThrow(BoviAPIError);
  });
...
describe("Rail Neutrality", () => {
  const mockQuotes: RailQuote[] = [
    { rail: "SEPA", fee: 0.5, etaSec: 3600, successP90: 0.998 },
    { rail: "FPS", fee: 0.6, etaSec: 10, successP90: 0.995 },
    { rail: "Card", fee: 2.9, etaSec: 5, successP90: 0.992 },
  ];
  ...
  test("auditRailSelection throws error for invalid rail", async () => {
    await expect(auditRailSelection("NonExistent", mockQuotes)).rejects.toThrow(BoviAPIError);
  });
...
describe("Local Knowledge Capture", () => {
  test("computeLocalIndex performs fast calculation", async () => {
    ...
    const result = await computeLocalIndex([{ price: 2.5 }, { price: 1.2 }, { price: 3.8 }]);
...
describe("Appeal & Liability Hooks", () => {
  ...
  test("getAppealStatus throws error for non-existent appeal", async () => {
    await expect(getAppealStatus("non-existent")).rejects.toThrow(BoviAPIError);
  });
